...
class ClubNightSession:
    ...
    """
    def __init__(self, player_names, num_courts, weights=None):
        ...
        self.player_ratings = {name: 0.0 for name in player_names}
        ...
        self._rest_rotation_queue = list(self.player_names)
    ...
    def prepare_round(self):
        ...
        num_to_rest = len(self.player_names) - num_players_to_play
        ...
        self._rest_rotation_queue = self._rest_rotation_queue[num_to_rest:] + self._rest_rotation_queue[:num_to_rest]
        ...
        for court_num, winning_team in winners_by_court.items():
            for player in winning_team:
                if player in self.player_ratings:
                    self.player_ratings[player] += 1

        # Add a small rating boost to resting players
        for player in self.resting_players:
            if player in self.player_ratings:
                self.player_ratings[player] += 0.5
    ...
    def get_standings(self):
        ...
        return sorted(self.player_ratings.items(), key=lambda item: item[1], reverse=True)
